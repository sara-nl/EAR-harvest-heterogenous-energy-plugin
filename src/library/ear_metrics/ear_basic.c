...
long long basic_acum_values[EAR_BASIC_EVENTS_SETS];
...
void init_basic_metrics()
{
	...
	for (sets=0;sets<EAR_BASIC_EVENTS_SETS;sets++){
		...
		cache_attach_opt[sets].attach.eventset=basic_event_sets[sets];
		...
		if ((retval=PAPI_add_named_event(basic_event_sets[sets],"iix86arch::UNHALTED_CORE_CYCLES"))!=PAPI_OK){
				fprintf(stderr,"basic_metrics: PAPI_add_named_event %s.%s\n","ix86arch::UNHALTED_CORE_CYCLES",PAPI_strerror(retval));
		}
		if ((retval=PAPI_add_named_event(basic_event_sets[sets],"ix86arch::INSTRUCTION_RETIRED"))!=PAPI_OK){
...
void reset_basic_metrics()
{
	...
	for (sets=0;sets<EAR_BASIC_EVENTS_SETS;sets++){
		if ((retval=PAPI_reset(basic_event_sets[sets]))!=PAPI_OK){
			fprintf(stderr,"basic_metrics: reset_basic_metrics.%s\n",PAPI_strerror(retval));
		}
...
void start_basic_metrics()
{
	...
	for (sets=0;sets<EAR_BASIC_EVENTS_SETS;sets++){
		if ((retval=PAPI_start(basic_event_sets[sets]))!=PAPI_OK){
			fprintf(stderr,"basic_metrics:start_basic_metrics.%s\n",PAPI_strerror(retval));
		}
	}
}
	...
	*instructions=0;
	...
	for (sets=0;sets<EAR_BASIC_EVENTS_SETS;sets++){
		if ((retval=PAPI_stop(basic_event_sets[sets],(long long *)&basic_values[sets]))!=PAPI_OK){
			fprintf(stderr,"basic_metrics: stop_basic_metrics.%s\n",PAPI_strerror(retval));
		}else{
