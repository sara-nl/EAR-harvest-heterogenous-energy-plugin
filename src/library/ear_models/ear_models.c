...
void init_power_policy()
{
    ...
    ear_verbose(1,
                "EAR: power policy configuration: policy %s performance penalty %lf accepted th %lf Reset_freq=%u performance gain %lf\n",
                (power_model_policy == MONITORING_ONLY) ? "MONITORING_ONLY" : ((power_model_policy ==
            ...
            if (coefficients[ref][i].available) {
                ...
                if (ear_my_rank == 0) {
                    if (ref == EAR_default_pstate) {
                        ear_debug(4,
                                  "EAR POLICY:::: EAR[mpi_rank=%d at %u] REFERENCE_COMPUTED ENERGY(%u) %lf (%lf,%lf) PROJECTED ENERGY(%u) %lf (%lf,%lf) (T_max %lf)\n",
                                  ear_my_rank, ear_frequency, EAR_default_frequency, E_ref, T_ref, P_ref,
                                  coefficients[ref][i].pstate, EP, TP, PP, T_max);
                    } else {
                        ear_debug(4,
                                  "EAR POLICY:::: EAR[mpi_rank=%d at %u] REFERENCE_PROJECTED ENERGY(%u) %lf (%lf,%lf) PROJECTED ENERGY(%u) %lf (%lf,%lf) (T_max %lf)\n",
                                  ear_my_rank, ear_frequency, EAR_default_frequency, E_ref, T_ref, P_ref,
                                  coefficients[ref][i].pstate, EP, TP, PP, T_max);
                    }
                }
        ...
        if (bestPstate > min_pstate) { // ref=1 is nominal 0=turbo, we are not using it
            ...
            while (try_next && (i >= min_pstate)) {
                if (coefficients[ref][i].available) {
                    ...
                    TP = time_projection(my_app, i, CPIP);
                    set_performance_projection(i, TP, PP, CPIP);
                    freq_gain = performance_gain * (double) (coefficients[ref][i].pstate - bestPstate) /
                                (double) bestPstate;
                    ...
                    if (perf_gain >= freq_gain) { // OK
						...
						freq_gain %lf (current %u next %u) (cpi actual %lf cpi proj %lf) GBS %lf\n",
                                  ear_my_rank, bestPstate, T_current, TP, perf_gain, freq_gain, bestPstate,
                                  coefficients[ref][i].pstate, my_app->CPI, CPIP, my_app->GBS_f0);
                        bestPstate = coefficients[ref][i].pstate;
                        T_current = TP;
                        i--;
                    } else {
                        if ((bestPstate != ear_frequency) || (bestPstate == EAR_default_frequency)) {
                            ear_debug(4,
                                      "EAR[MIN_TIME][rank %d] Not scaling more than -> Tcurrent[%u]= %lf vs Tproj[%u]= %lf (perf_gain %lf) \n",
...
unsigned int performance_projection_ok(projection_t *PREDICTION, struct App_info *SIGNATURE) {
    if (equal_with_th(PREDICTION->Time, SIGNATURE->seconds, performance_penalty_th) &&
        equal_with_th(PREDICTION->Power, SIGNATURE->POWER_f0, performance_penalty_th)) {
