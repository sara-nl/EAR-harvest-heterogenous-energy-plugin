...
#define WARNING2_DEF_PSTATES_LESS 0
...
state_t policy_set_risk(policy_conf_t *ref,policy_conf_t *current,risk_t risk_level,ulong opt_target,ulong mfreq,ulong *nfreq,ulong *f_list,uint nump)
{
  ...
  if (risk2){
    debug("We are in risk2");
		...
		max_pstate=max_pstate+WARNING1_MAX_PSTATES_LESS+WARNING2_MAX_PSTATES_LESS;
  ...
  if (panic){
		...
		max_pstate=max_pstate+WARNING1_MAX_PSTATES_LESS+WARNING2_MAX_PSTATES_LESS+PANIC_MAX_PSTATES_LESS;
	...
	f=frequency_pstate_to_freq_list(max_pstate,f_list,nump);
  *nfreq=f;
