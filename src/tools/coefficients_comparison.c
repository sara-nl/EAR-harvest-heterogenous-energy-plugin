...
int compare_coefficients(coefficient_t *avg,coefficient_t *per_node)
{
	...
	if (are_equal) return IS_EQUAL;
	else return IS_DIFFERENT;
	
}

void accum_coeffs(coefficient_t *avg,coefficient_t *new)
{
	int i;
	for (i=0;i<num_pstates;i++){
		avg[i].pstate_ref+=new[i].pstate_ref;
		avg[i].pstate+=new[i].pstate;
		avg[i].available+=new[i].available;
		avg[i].A+=new[i].A;
		avg[i].B+=new[i].B;
		avg[i].C+=new[i].C;
		avg[i].D+=new[i].D;
