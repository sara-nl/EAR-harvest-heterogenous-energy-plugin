...
void fill_list_p_states(uint turbo,uint nom_freq,uint num_node_p_states)
{
    int num_pstates = num_node_p_states;


    MALLOC(node_freq_list, uint, num_pstates);

	node_freq_list[0]=turbo;
    for (i = 1; i < num_pstates; i++)
    {
        node_freq_list[i] = nom_freq-((i-1)*100000);
    }
...
void usage(char *app)
{
    fprintf(stdout, "Usage: %s db_name coefficients_db turbo nominal num_pstates nodename\n", app);
	fprintf(stdout, "db_name: filename with metrics collected during the learning phase\n");
	fprintf(stdout, "coefficients_db: first part of the coefficient filename \n");
	fprintf(stdout, "turbo: frequency value describing turbo frequency\n");
	fprintf(stdout, "nominal frequency: in KHertz (2.4GHz --> 2400000)\n");
	fprintf(stdout, "num_pstates: total number of p_states to  be taken into account. Including turbo if available by BIOS\n");
	fprintf(stdout, "nodename: concatenated to coefficients_db filename\n");
    exit(1);
}
...
int main(int argc, char *argv[])
{
    ...
    //
	num_node_p_states=atoi(argv[5]);
	nom_freq=atoi(argv[4]);
	turbo=atoi(argv[3]);
	if (num_node_p_states>2) min_freq = nom_freq-((num_node_p_states-2)*100000);
	else min_freq=nom_freq;
    nodename = argv[6];

    // We get how many samples per frequency we have
    fill_list_p_states(turbo,nom_freq,num_node_p_states);
