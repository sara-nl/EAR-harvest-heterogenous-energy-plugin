...
int main(int argc, char *argv[])
{
    ...
    for (i = 0; i < num_node_p_states; i++) {
		application_t *aapps = sorted_app_list[i];
		for (f = 0; f < current[i]; ++f) {
			printf("%lu\t%0.2lf\t%0.2lf\t%0.2lf\t%s\n",
				aapps[f].signature.def_f, aapps[f].signature.CPI, aapps[f].signature.TPI, aapps[f].signature.DC_power, aapps[f].job.app_id);
		}
	}
