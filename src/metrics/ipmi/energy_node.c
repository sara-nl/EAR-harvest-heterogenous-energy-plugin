...
int read_dc_energy_time(energy_handler_t *eh, ulong *energy, ulong *time_ms) {
	...
	if (node_energy_ops.read_dc_energy_time != NULL)
		return node_energy_ops.read_dc_energy_time(eh->ctx, energy, time_ms);
	else {
		*energy = 0;
		*time_ms = 0;
		return -1;
...
int
read_dc_energy_time_debug(energy_handler_t *eh, ulong *energy_j, ulong *energy_mj, ulong *time_sec, ulong *time_ms) {
	debug("read_dc_energy_time_debug");
	if (node_energy_ops.read_dc_energy_and_time != NULL)
		return node_energy_ops.read_dc_energy_and_time(eh->ctx, energy_j, energy_mj, time_sec, time_ms);
	else {
		*energy_j = 0;
		*energy_mj = 0;
		*time_sec = 0;
...
int read_dc_energy_time_debug_try(energy_handler_t *eh, ulong *energy_j, ulong *energy_mj, ulong *time_sec,
								  ulong *time_ms) {
	int tries = 0;
	int ret;
	*energy_j = 0;
