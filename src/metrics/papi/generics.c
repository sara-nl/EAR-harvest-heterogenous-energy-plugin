...
void metrics_get_app_name(char *app_name)
{
	...
	{
		verbose(0, "executable info not available, exiting");
		exit(2);
	}
...
int _papi_start_counters(int event_set)
{
	...
	if ((result = PAPI_start(event_set) != PAPI_OK)) {
		verbose(0, "ERROR while starting accounting (%s)", PAPI_strerror(result));
		return 0;
	}
...
int _papi_stop_counters(int event_set, long long *event_values)
{
	...
	if ((result = PAPI_stop(event_set, event_values)) != PAPI_OK) {
		verbose(0, "ERROR while stopping accounting (%s)", PAPI_strerror(result));
		return 0;
	}
	return 1;
}
...
int _papi_read_counters(int event_set, long long *event_values)
{
	...
	if ((result = PAPI_read(event_set, event_values)) != PAPI_OK) {
		verbose(0, "ERROR while reading counters (%s)", PAPI_strerror(result));
		return 0;
	}
	return 1;
}
...
int _papi_event_add(int event_set, char *event_name)
{
	...
	if ((result = PAPI_add_named_event(event_set, event_name)) != PAPI_OK) {
		verbose(0, "ERROR while adding %s event (%s)", event_name, PAPI_strerror(result));
		return 0;
	}
	return 1;
}
...
int _papi_multiplex_init()
{
	...
	if ((papi_multi = PAPI_multiplex_init()) != PAPI_OK){
		verbose(0,"WARNING when multiplexing %s", PAPI_strerror(papi_multi));
		return 0;
	}
	return 1;
}
...
int _papi_component_init(char *component_name)
{
	...
	if ((cid = PAPI_get_component_index(component_name)) < 0) {
		verbose(0, "ERROR, %s component not found (%s)", component_name, PAPI_strerror(cid));
		return 0;
	}
	if ((info = PAPI_get_component_info(cid)) == NULL) {
		verbose(0, "ERROR, error while accessing to component %s info", component_name);
		return 0;
	}
	if (info->disabled) {
		verbose(0, "ERROR, %s disabled (%s)", component_name, info->disabled_reason);
		return 0;
	}
...
int _papi_init()
{
	...
	if ((papi_init = PAPI_library_init(PAPI_VER_CURRENT)) != PAPI_VER_CURRENT) {
		verbose(0, "ERROR intializing PAPI (%s), exiting", PAPI_strerror(papi_init));
		exit(1);
	}
