...
struct energy_gpu_ops
{
	...
	state_t (*count)		(ctx_t *c);
	state_t (*pool)			(void  *c);
	state_t (*read)			(ctx_t *c, gpu_t *data);
	state_t (*read_copy)	(ctx_t *c, gpu_t *data, gpu_t data_diff);
...
state_t gpu_load()
{
	...
	if (state_ok(nvml_status()))
	{
		...
		ops.read		= nvml_read;
		ops.read_copy	= nvml_count;
		ops.data_diff	= nvml_count;
...
{
	preturn (ops.copy, c, data, data_diff);
}
