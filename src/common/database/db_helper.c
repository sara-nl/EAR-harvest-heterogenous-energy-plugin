...
//        sprintf(query,  "SELECT Learning_applications.* FROM Learning_applications INNER JOIN "\
        ...
        sprintf(query,  "SELECT Learning_applications.* FROM Learning_applications WHERE job_id > %d AND node_id='%s' ORDER BY job_id "\
                        "LIMIT %d", current_job_id, node_name, max_apps);
