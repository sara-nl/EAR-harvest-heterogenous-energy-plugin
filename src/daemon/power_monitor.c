...
void powermon_new_max_freq(ulong maxf)
{
	...
	if ((ccontext>=0) && (current_ear_app[ccontext]->app.job.id>0) && (current_ear_app[ccontext]->app.is_mpi==0)){
		if (maxf<current_node_freq){
			verbose(VJOBPMON,"MaxFreq: Application is not mpi, automatically changing freq from %lu to %lu",current_node_freq,maxf);
  ...
  uint ps_def,ps_max;
	...
	if ((ccontext>=0) && (current_ear_app[ccontext]->app.is_mpi==0)){
		if (def_freq<current_node_freq){
			verbose(VJOBPMON,"RedFreq: Application is not mpi, automatically changing freq from %lu to %lu",current_node_freq,def_freq);
