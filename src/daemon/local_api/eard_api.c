...
state_t eards_mgt_imcfreq_set_current( ulong max_khz, ulong min_khz)
{
  ...
  {
    if (warning_api(my_write(ear_fd_req[freq_req],(char *)&req, sizeof(req)), sizeof(req),
       "while asking for setting uncore freq limits")) state_return_msg(EAR_ERROR,errno,"Asking for setting uncore freq limits");

    if (warning_api(my_read(ear_fd_ack[freq_req], (char *)&ack, size), size,
      "receiving ack while setting uncore freq ")) state_return_msg(EAR_ERROR,errno,"Receiving ack while setting uncore freq");
  }
  return EAR_SUCCESS;
