...
{      	
	...
	ear_verbose(0,"creating file %s for shared memory\n",buff);
	fd=open(buff,O_CREAT|O_RDWR|O_TRUNC,S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP | S_IROTH | S_IWOTH);
	if (fd<0){
		ear_verbose(0,"eard: error creating sharing memory (%s)\n",strerror(errno));
		umask(my_mask);
		return NULL;
	}
	ear_verbose(0,"shared file for mmap created\n");
	umask(my_mask);
	// Default values
	my_conf.max_freq=max_freq;
	my_conf.th=PERFORMANCE_GAIN;
	my_conf.force_rescheduling=0;
	ear_verbose(0,"writting default values\n");
	...
	ear_verbose(0,"mapping shared memory\n");
...
ear_conf_t * attach_ear_conf_shared_area(char * ear_conf_path)
{
    ...
    fd=open(buff,O_RDWR);
    if (fd<0){
        ear_verbose(0,"error attaching to sharing memory (%s)\n",strerror(errno));
        return NULL;
    }
    my_shared_region= mmap(NULL, sizeof(my_conf),PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0);
    if ((my_shared_region == MAP_FAILED) || (my_shared_region == NULL)){
