...
--------------------------------------
A set of scripts are provided for speed up with minimum edition requirements. These files are placed in the `bin/scripts` folder in your EAR installation folder. For a list of all provided scripts visit [EAR scripts page](https://github.com/BarcelonaSupercomputingCenter/EAR/blob/development/etc/scripts/README.md).

The compiling script is located in `bin/scripts/learning/learning_phase_compile.sh`. Before execute it, you have to perform some adjustments:
...
1) Open the script `bin/scripts/learning/learning_phase_execute.sh`.
...
5) Execute the learning phase in all of your nodes by typing a command like: `./learning_phase_execute.sh <hostlist>`, passing a the path of a file containing the list of nodes where you want to perform the learning phase. An `sbatch` will be launched exclusively in every node, performing a `srun` series of the kernel in the same node.
6) Execute the coefficients compute binary by typing `./learning_phase_coeffs.sh <hostlist>` in a node which shares the same architecture (or at least the P_STATEs list) of the nodes of the completed learning phase.
