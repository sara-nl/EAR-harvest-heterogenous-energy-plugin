...
export EAR_APP_NAME=lu-mz
mkdir -p EAR_OUTS
export EAR_USER_DB_PATHNAME=EAR_OUTS/${EAR_APP_NAME}.${FREQ}.${EAR_POWER_POLICY}.${EAR_PERFORMANCE_PENALTY}
export BENCHMARK=$EAR_INSTALL_PATH/bin/kernels/lu-mz.C.$MPIS
export MKL_NUM_THREADS=$THREADS
export OMP_NUM_THREADS=$THREADS
echo "Executing lu-mz(with EAR) with $MPIS mpis and $THREADS threads in $NODENAME at f=$FREQ"
