...
if [ "$#" -eq "0" ]
then
echo "$0 freq sockets cores_per_socket total_cpus [ear|baseline|lp|lp_turbo]"
exit 1
fi
...
export EAR_APP_NAME=stream
mkdir -p EAR_OUTS
export EAR_USER_DB_PATHNAME=EAR_OUTS/${EAR_APP_NAME}.${FREQ}.${EAR_POWER_POLICY}.${EAR_PERFORMANCE_PENALTY}
export BENCHMARK=$EAR_INSTALL_PATH/bin/kernels/stream_mpi
export MKL_NUM_THREADS=$THREADS
export OMP_NUM_THREADS=$THREADS
echo "Executing estream with EAR) with $MPIS mpis and $THREADS threads in $NODENAME at f=$FREQ"
