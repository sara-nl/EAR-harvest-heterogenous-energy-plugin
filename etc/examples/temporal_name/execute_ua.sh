...
export EAR_APP_NAME=ua
mkdir -p EAR_OUTS
export EAR_FILE_NAME=EAR_OUTS/${EAR_APP_NAME}.${FREQ}.${EAR_POWER_POLICY}.${EAR_PERFORMANCE_PENALTY}
export BENCHMARK=$EAR_INSTALL_PATH/bin/kernels/ua.C.x
export MKL_NUM_THREADS=$THREADS
export OMP_NUM_THREADS=$THREADS
echo "Executing ua(with EAR) with $MPIS mpis and $THREADS threads in $NODENAME at f=$FREQ"

# #adding bash tool library
source $EAR_INSTALL_PATH/conf/EAR_scripts.sh
source $EAR_INSTALL_PATH/WORKLOADS/Bash_utils.sh
