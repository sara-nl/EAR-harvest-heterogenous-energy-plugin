...
export EAR_APP_NAME=sp-mz
export EAR_USER_DB_PATHNAME=EAR_OUTS/${EAR_APP_NAME}.${FREQ}.${EAR_POWER_POLICY}.${EAR_PERFORMANCE_PENALTY}
mkdir -p EAR_OUTS
export BENCHMARK=$EAR_INSTALL_PATH/bin/kernels/sp-mz.C.$MPIS
export MKL_NUM_THREADS=$THREADS
export OMP_NUM_THREADS=$THREADS
echo "Executing sp-mz(with EAR) with $MPIS mpis and $THREADS threads in $NODENAME at f=$FREQ"
