...
#define	EAR_ACUM_TOT_CYC 	0 // Cycles
#define EAR_ACUM_TOT_INS 	1 // Instructions
#define EAR_ACUM_LD_INS 	2 // Load instructions
#define EAR_ACUM_SR_INS 	3 // Store instructions
#define EAR_ACUM_DRAM_ENER 	4 // RAPL DRAM energy
#define EAR_ACUM_PCKG_ENER 	5 // RAPL CORES+CACHE energy
#define EAR_ACUM_DC_ENER	6 // IPMI NODE energy
#define EAR_ACUM_L1_CACHE	7 // L1 cache misses
#define EAR_ACUM_L2_CACHE	8 // L2 cache misses
#define EAR_ACUM_L3_CACHE	9 // L3 cache misses
...
static ulong metrics_ipmi;
static ulont metrics_freq;
...
int metrics_init(int my_id, int pid)
{
	...
	if (hw_general == NULL) {
		ear_verbose(0 ,"EAR: WARNING PAPI_get_hardware_info is returning NULL\n");
	} else {
	...
	ipmi_size = ear_daemon_client_node_energy_data_size();
	...
	ipmi_elements = ipmi_size / sizeof(ulong);

	metrics_bandwith = malloc(uncore_size);
	metrics_ipmi = malloc(ipmi_size);
	metrics_rapl = malloc(rapl_size);


	if (metrics_bandwith == NULL || metrics_ipmi == NULL || metrics_rapl == NULL) {
	...
	if (papi_flops_supported)
	{
		...
		if (metrics_flops == NULL || metrics_flops_weight == NULL) {
			ear_verbose(0,"EAR: Error allocating memory in %s metrics\n", __FILE__);
			exit(1);
		}
	 ...
	 ulong energy_uj, uint N_iters, ulong time_us)
{
	...
	// TODO: IF GLOBAL (whole app)
		...
		// Cache
		app->L1_misses = metrics_l1;
		app->L2_misses = metrics_l2;
		app->L3_misses = metrics_l3;
...
void acum_counters()
{
	...
	accum_metrics[EAR_ACUM_DC_ENER] += metrics_ipmi;
}
...
void metrics_stop()
{
	...
	stop_cache_metrics(&metrics_l1, &metrics_l2, &metrics_l3);
	...
	metrics_freq = ear_daemon_client_end_compute_turbo_freq();

	// Energy metrics (special treatment)
	ear_daemon_client_node_dc_energy(&metrics_aux);
	metrics_ipmi = metrics_aux - metrics_ipmi;
}

void metrics_start()
{
	ear_daemon_client_node_dc_energy(&metrics_ipmi);
...
application_t* metrics_end_compute_signature(uint N_iters, ulong min_time_us)
{
	application_t *app;
